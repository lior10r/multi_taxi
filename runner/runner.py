...
class ParallelEnvRunner:
    ...
    def train(self):
        ...
        tune.run(
            ...
            stop={"training_iteration": 100000},  # {"timesteps_total": 5000000},
            checkpoint_freq=10,
    ...
    def evaluate(self, algorithm, checkpoint_path: str = None, seed: int = 42, num_of_tries: int = 100):
        success = False
        for _ in range(num_of_tries):
            if success:
                break

            os.system('cls')  # on windows, in linux: os.system('clear')
            ...
            while True:
                ...
                if all(term.values()):
                    if Event.OUT_OF_TIME not in info[list(info.keys())[0]]['events']:
                        success = True
                    print("Terminating", success)
                    break
...
if __name__ == "__main__":
    ...
    runner = ParallelEnvRunner(MultiTaxiCreator.get_env_name(), MultiTaxiCreator.create_env(), 
                               DQNCreator.get_algo_name(), DQNCreator.get_config(MultiTaxiCreator.get_env_name()))
    if args.mode == 'train':
        runner.train()
    elif args.mode == 'evaluate':
        runner.evaluate(DQNCreator.get_algo(), args.checkpoint_path)
